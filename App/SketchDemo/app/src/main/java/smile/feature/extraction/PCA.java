...
public class PCA implements LinearProjection, Serializable {
     ...
     * @param dimension choose top dimension principal components used for projection.
     */
    public void setProjection(int dimension) {
        if (dimension < 1 || dimension > inputDimension) {
            throw new IllegalArgumentException("Invalid dimension of feature space: " + dimension);
        }

        this.featureDimension = dimension;
        projection = new Matrix(dimension, inputDimension);
    ...
    public double[] project(double[] x) {
        if (x.length != inputDimension) {
            throw new IllegalArgumentException(String.format("Invalid input vector size: %d, expected: %d", x.length, inputDimension));
        }
    ...
    public double[][] project(double[][] x) {
        if (x[0].length != mu.length) {
            throw new IllegalArgumentException(String.format("Invalid input vector size: %d, expected: %d", x[0].length, inputDimension));
        }

        double[][] y = new double[x.length][featureDimension];
