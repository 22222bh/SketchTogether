...
public class Classifier {
    ...
    private List<String> classify(DataPoint point){ // 외부에서 들어옴.
        HashMap<Category, Integer> hashMap = new HashMap<>(); // 초기 용량 지정 가능!!! 나중에 전체 개수 정해지면 초기용량 ㄱㄱ
        List<DataInfo> listDistance = calculateDistances(feature);
        for (int i = 0; i < K; i++){
            ...
            listDistance.set(minIndex, Double.MAX_VALUE); // 제일 가까운 애를 맥스로 갱신 - 다음 번에는 얘는 자연스럽게 제외됨.
        }
